...
public class SolverGreedyGenerateur extends Solver {
	...
	public void solve() {
		...
		while (iterMaison.hasNext() && !Thread.currentThread().isInterrupted()) {
			...
			if (gen != null && reseau.getTauxUtilisation().get(gen.getNom()) >= 1.0) {
				genArray.remove(gen);
			}
