 ...
 * <p>Il explore systématiquement (en profondeur) les connexions possibles, gardant en mémoire le surcharge sur la branche courante. Dès que cela dépasse la meilleure coût total vu avant, on arrête d'explorer la branche courante : elle ne peut pas contenir la solution optimale.</p>
...
public class SolverBranchBound extends Solver {
    ...
    public void solve() {
        ...
        // Sinon, on part de la configuration existante. (Comme ça on est extensible, et refait pas de travail si on a déjà la solution optimale.)
        ...
        // on prépare des tableaux (plus rapides dans ce cas précis).
        ...
        if (!this.meilleureConfiguration.isEmpty()) {
            ...
            System.out.println("Branch & Bound terminé en " + duree + " ms avec coût minimal trouvé : " + this.meilleurCoutGlobal);
        }
    }

    /**
     * Partie récursive de l'algorithme qui exécute la recherche dans l'arbre des possibilités.
     * @param index L'indice de la maison à traiter (profondeur dans l'arbre).
     * @param lambda Le coefficient de pénalité (λ) appliqué à chaque unité de surcharge.
     * @param surchargePartielle La somme des surcharges (en ratio) accumulée sur la branche courante.
     */
    ...
    /**
     * Décide si la branche explorée jusqu'à une feuille a donné un meilleur coût que le record actuel.
     * Calcule le coût complet (Dispersion + Surcharge).
     * @param lambda Le coefficient de pénalité (λ) appliqué à chaque unité de surcharge.
     * @param totalSurcharge La somme des surcharges (en ratio) accumulée sur la branche.
     */
